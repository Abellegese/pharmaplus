...
from pharmaplus.helpers import (
  print_batch_results_rich_table,
  LOG_2PI,
  FAMILY_TO_TYP,
  _RDKIT_FACTORY,
)
...
def lig_pharm_from_mol(
  mol: Chem.Mol, confId: int = -1, max_nodes: int = 48
...
def generate_conformers_from_smiles(
  smiles: str, n_confs: int, seed: int, optimize: bool
) -> Chem.Mol:
...
def to_device_pharm(
  ph: Dict[str, torch.Tensor], device: torch.device
) -> Dict[str, torch.Tensor]:
...
def score_gt_confs(
  ...
  with torch.amp.autocast(device_type=device.type, enabled=use_amp):
    stA = model.precompute_stageA(
      ...
      p_pos,
      pocket["x"].to(device),
      pocket["typ"].to(device),
      pocket["score"].to(device),
      pocket["rad"].to(device),
      p_mask,
      topk=topk,
      edge_pairs=edge_pairs,
    )
...
def score_best_other_ligand_on_same_pocket(
  ...
  conf_mol = generate_conformers_from_smiles(
    other_smiles, n_confs=n_confs, seed=seed, optimize=optimize
  )
...
def main():
  ...
  ap.add_argument(
    "--other_dir",
    required=True,
    help="Directory containing many other *.pt to sample from",
  )
  ap.add_argument("--other_n", type=int, default=4)
  ...
  edge_pairs = int(
    args.edge_pairs if args.edge_pairs is not None else edge_pairs_default
  )
  ...
  cent_np = (
    p_pos.mean(axis=0).astype(np.float32) if p_pos.size else np.zeros((3,), np.float32)
  )
  ...
  pbar = tqdm(
    ...
    total=len(picks),
    desc="Scoring ligands vs pocket",
    unit="lig",
    dynamic_ncols=True,
    ascii="░▒▓█",
    bar_format=(
  ...
  for _, pt in pbar:
    try:
      _, other_sdf = load_item_paths(pt)
  ...
  results.append((base["score_retr"], args.item_pt, "", base))
  results.sort(key=lambda x: x[0], reverse=True)
  print_batch_results_rich_table(
    results,
    top_n=20,
    title=f"Other ligands vs pocket ({len(results)} sampled) | lambda_retr={args.lambda_retr}",
  )
